...
def process_paragraph(para, src_lang, model, target_lang, api_token, api_url, force_proofread=False):
    try:
        ...
        if paragraph_text.strip():
            result_text = translate_or_proofread(paragraph_text, src_lang, target_lang, model, api_token, api_url, force_proofread=force_proofread)
            # Clear existing runs
...
def process_document(input_file, output_file, model, target_lang, api_token, src_lang=None, api_url='http://localhost:11434/api/generate', force_proofread=False):
    try:
        ...
        for para in doc.paragraphs:
            process_paragraph(para, src_lang, model, target_lang, api_token, api_url, force_proofread=force_proofread)
